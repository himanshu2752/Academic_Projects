...
for (i in 1:5)
  ...
  fit_NB <- naiveBayes(as.formula(paste0("as.factor(", colnames(spam_dataframe)[col_val], ") ~ .")), data = trainInstances, threshold = x)
...
#  x <- threshold
  ...
  cat("accuracy of NB for threshold = ",x," is :" , nb_accuracy,"\n")
}
}
...
for (i in 1:5)
{
  ...
  fit_SVM <- svm(f, data=trainInstances, cost	=	y,	gamma	=	1,kernel = "linear")
  ...
  cat("accuracy of SVM for Cost: ", y, "is =" , svm_accuracy,"\n")
}
}
...
for (i in 1:5){
  ...
  n <- names(trainInstances)
  f <- as.formula(paste0("as.factor(", colnames(spam_dataframe)[col_val], ") ~ ."))
  fit_DT <- rpart(f,  data = trainInstances,  method="class",parms	=	list(split	=	"information"),minsplit = 6, minbucket = 2)
  pred <- predict(fit_DT,testInstances, type="class")
  pruneDT <- prune(fit_DT,cp=0.01)
  ...
  cat("accuracy of DT for cp = ",z," is = ",DT_accuracy,"\n")
}
}
...
for (i in 1:5){
  ...
  for (j in 1:5 )
  {
    ...
    cat(" accuracy of Perceptron for threshold - ",v," is = ",p_accuracy,"\n")
...
for (i in 1:5){
  ...
  nu_net <- as.formula(paste(colnames(trainInstances)[as.integer(col_val)],"~", paste(n[!n %in% colnames(trainInstances)[as.integer(col_val)]], collapse = " + ")))
  net<-paste(c(n[!n %in% colnames(trainInstances)[as.integer(col_val)]]))
  nn_fit <- neuralnet(nu_net ,trainInstances, hidden = z, threshold = 0.1)
  temp_test <- subset(testInstances, select = c(net))
  ...
  cat(" accuracy of NN for hidden layers - ",z," is = ",nn_accuracy,"\n")
